...
from src.inventory.infrastructure.repositories import ItemRepository, WarehouseRepository
...
async def test_end_to_end_anomaly_detection_flow():
    async with AsyncSessionLocal() as session:
        item = Item(sku=SKU(value=f"SKU-{uuid.uuid4().hex[:6].upper()}"), description="Anomaly Test Item", is_active=True)
    ...
    async with LifespanManager(app):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            # Check DB
            async with AsyncSessionLocal() as session:
                result = await session.execute(text("SELECT id FROM movements WHERE id = :id"), {"id": movement_id})
                row = result.fetchone()
