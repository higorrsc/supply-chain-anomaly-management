...
def upgrade():
    ...
    op.alter_column(
        "users", "some_column", type_=sa.String(50), existing_type=sa.Integer()
    )


def downgrade():
    op.alter_column(
        "users", "some_column", type_=sa.Integer(), existing_type=sa.String(50)
    )
```
...
def downgrade():
    op.alter_column("users", "some_column", new_column_name="some_column_new")
